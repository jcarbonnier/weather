...
RSpec.describe Public::WeatherAveragePolicy, type: :policy do
  subject(:current_policy) { described_class }

  describe '#limit params' do
    let(:user) { build(:user) }

    it 'grants index params' do
      expect(current_policy.new(user, WeatherAverage).permitted_params_for_index).to eq(%w[q])
    end
  end
    ...
    context 'when user is missing' do
      ...
      permissions :index? do
        it 'grants access if current is nil' do
          expect(current_policy).to permit
        end
      end
